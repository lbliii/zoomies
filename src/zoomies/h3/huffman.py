"""Huffman decoding for HPACK/QPACK (RFC 7541 Appendix B).

Provides decode-only support. Encoding without Huffman is spec-compliant
and avoids the complexity of the encoding path.
"""
...
def huffman_decode(data: bytes) -> bytes:
    ...
    Raises ValueError on invalid or incomplete Huffman sequences.
    """
    result = bytearray()
    bits = 0  # accumulated bit pattern
    nbits = 0  # number of accumulated bits

    for byte in data:
        bits = (bits << 8) | byte
        nbits += 8

        # Try to emit symbols from accumulated bits
        while nbits >= 5:  # shortest code is 5 bits
            found = False
            # Check all possible code lengths (5 to min(nbits, 30))
            for length in range(5, min(nbits, 30) + 1):
                # Extract the top `length` bits
                shift = nbits - length
                candidate = (bits >> shift) & ((1 << length) - 1)
                sym = _DECODE_TABLE.get((candidate, length))
                if sym is not None:
                    result.append(sym)
                    nbits -= length
                    bits &= (1 << nbits) - 1
